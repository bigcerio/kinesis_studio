...
const categoryColor: Record<Exercise["category"], string> = {
  ...
  recupero: "bg-teal-100 text-teal-700 dark:bg-teal-900 dark:text-teal-300",
};
...
function ExerciseVideo({ exercise }: { exercise: Exercise }) {
  ...
  if (exercise.videoSource === "youtube" && exercise.videoId) {
    ...
    return (
      <button
        ...
        className="group relative flex aspect-video w-full items-center justify-center overflow-hidden bg-slate-900 print:hidden"
      >
        <img
  ...
  return (
    <a
      ...
      className="group flex aspect-video items-center justify-center bg-slate-100 print:hidden dark:bg-slate-800"
    >
      <span className="flex items-center gap-2 text-xs font-medium text-slate-500 transition-colors group-hover:text-slate-800 dark:group-hover:text-slate-200">
        <span className="flex h-9 w-9 items-center justify-center rounded-full bg-white shadow dark:bg-slate-950">▶</span>
        Cerca l&apos;esecuzione
      </span>
    </a>
  );
  ...
  if (groupIds.length === 0) {
    return (
      <div className="flex min-h-dvh flex-col items-center justify-center gap-4 bg-slate-50 px-6 text-center dark:bg-slate-950">
        <p className="text-sm text-slate-500">Nessun distretto selezionato per la scheda.</p>
        <Link
          href="/corpo"
          className="rounded-full bg-blue-600 px-5 py-2.5 text-sm font-medium text-white dark:bg-blue-500 dark:text-blue-950"
        >
  ...
  return (
    <div className="relative min-h-dvh bg-slate-50 dark:bg-slate-950">
      {trial && (
        ...
        >
          <span className="rotate-[-28deg] whitespace-nowrap text-6xl font-bold text-slate-300/60 dark:text-slate-100/10 sm:text-8xl">
            ANTEPRIMA · KINESIS STUDIO
          </span>
        </div>
      )}

      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-6 py-4 print:hidden dark:border-slate-800">
        <div>
          <Link href="/corpo" className="text-xs text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">
            ← Mappa corporea
          </Link>
        ...
        <button
          ...
          className="rounded-full bg-blue-600 px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-blue-700 dark:bg-blue-500 dark:text-blue-950 dark:hover:bg-blue-400"
        >
          Esporta / Stampa PDF
        </button>
      </header>
      ...
      <main className="mx-auto max-w-3xl px-6 py-8">
                ...
                <li key={phase}>
                  ...
                  <ul className="mt-1.5 flex flex-col gap-1">
                    {exercises.map((ex) => (
                      ...
                      >
                        <span>{ex.name}</span>
                        <span className="shrink-0 text-xs text-slate-400">{ex.sets}</span>
                      </li>
                    ))}
                  </ul>
                </li>
        ...
        <div className="flex flex-col gap-8">
          {sections.map(({ group, exercises }) => (
            <section key={group.id}>
              <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100">{group.name}</h3>
              <p className="text-xs text-slate-500">{group.function}</p>
                  ...
                  {exercises.map((ex) => (
                    ...
                    >
                      ...
                      <div className="flex flex-1 flex-col gap-1.5 p-3.5">
                        ...
                        <p className="text-xs font-medium text-slate-700 dark:text-slate-200">{ex.sets}</p>
                        ...
                        {ex.videoSource === "youtube" && ex.videoId && (
                          <div className="mt-1 flex items-center gap-2 border-t border-slate-100 pt-2 dark:border-slate-800">
        ...
        <div className="mt-10 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
